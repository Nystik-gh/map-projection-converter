...
self.onmessage = function (e) {
  ...
  const srcW = e.data.width;
  const srcH = e.data.height;
  ...
  for (let y = 0; y < outH; y++) {
    ...
    const t = index - i;
    const plen = ROBINSON_X[i] + (ROBINSON_X[i + 1] - ROBINSON_X[i]) * t;
    const pdfe = ROBINSON_Y[i] + (ROBINSON_Y[i + 1] - ROBINSON_Y[i]) * t;
    const yRob = ROBINSON_Y_MAX * pdfe * (phi >= 0 ? 1 : -1);
    const py = (1 - yRob / ROBINSON_Y_MAX) * 0.5 * srcHmax;
    const outRowBase = y * outW * 4;

    for (let x = 0; x < outW; x++) {
      const lambda = (x / outW) * 2 * Math.PI - Math.PI;
      const xRob = 0.8487 * lambda * plen;
  ...
  self.postMessage({ out: out, width: outW, height: outH }, [out.buffer]);
};
